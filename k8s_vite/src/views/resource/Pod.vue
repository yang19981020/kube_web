  ...
  setup() {
    ...
    axios.get('/resource/pod_list').then((res) => {
      for (var i = 0; i < res.data.data[0].items.length; i++) {
        Pod_list.push({
          name: res.data.data[0].items[i].metadata.name,
          namespace: res.data.data[0].items[i].metadata.namespace,
          status: res.data.data[0].items[i].status.phase,
          startTime: res.data.data[0].items[i].status.startTime,
          podIP: res.data.data[0].items[i].status.podIPs,
          containerInfo: res.data.data[0].items[i].status.containerStatuses,
          qosClass: res.data.data[0].items[i].status.qosClass,
        })
      }
    })
    axios.get('/resource/namespace_list').then((res) => {
      for (var i = 0; i < res.data.data[0].items.length; i++) {
        Namespace_list.push({
          value: res.data.data[0].items[i].metadata.name,
          label: res.data.data[0].items[i].metadata.name,
        })
      }
    })
